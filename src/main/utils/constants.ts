...
export const DATA_DIR = process.env.CHYZCORD_USER_DATA_DIR ?? process.env.EQUICORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "ChyzcordData", suffix)
        : join(app.getPath("userData"), "..", "Chyzcord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Chyzcord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
